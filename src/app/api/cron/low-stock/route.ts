...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (ownerEmailSetting) {
      const html = `
        <h2 style="color:#E11D48">⚠️ Low Stock Alert — ${businessNameSetting?.value ?? 'Rosella Stores'}</h2>
        ...
        <p style="margin-top:20px;color:#6B7280;font-size:12px">This is an automated alert from Rosella Stores.</p>
      `;
