  ...
  try {
    ...
    if (session.user.role !== 'ADMIN' && new Date() < requestRow.earliestPermanentAt) {
      return NextResponse.json(
        { error: 'Permanent deletion is locked for 72 hours after soft delete' },
        { status: 400 }
      );
    }
    ...
    const msg = String((e as any)?.message || 'Internal server error');

    if (code === 'P2003' || msg.toLowerCase().includes('foreign key')) {
      return NextResponse.json({ error: 'Cannot permanently delete: record is still referenced.' }, { status: 400 });
    }

    if (code === 'P2025' || isRecordNotFound(e)) {
